...
# ---- Main QA Tab ----
    ...
    if query and selected_file:
        with st.spinner("Thinking..."):
            ...
            # Filter Weaviate chunks by the selected filename
            #filters = Filter.equal(path=["filename"], valueText=selected_file)

            for q in sub_queries:
                hybrid_input = HybridSearchToolInputSchema(query=q, top_k=5, filename=selected_file)
                results = hybrid_tool.run(hybrid_input) # Apply the filename filter here
